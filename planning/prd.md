...
> v0.4（2026-06-28）。数据完全本地、24h 被动听你说的一切、自动蒸馏成数字分身，并主动给建议/安抚/推荐的个人 AI 助手。安卓 App + Web 控制端；大脑跑本地电脑或云服务器。
...
- **v0.3**：推荐引擎——联网动态搜索(Bing/可切搜索API)真实结果→LLM 从中挑书/影/做事方式；反幻觉：item 须落地搜索结果、based_on 须引 persona 非空维度。
- **v0.4（本轮）**：个人 wiki——记忆→自动切片(原子化)+LLM 分类打标+编译互链主题页(title/body/tags/source_ids/links)；反幻觉：source_ids 须真实+body 落地源记忆；按标签/词检索。
